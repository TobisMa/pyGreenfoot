...
class Actor(metaclass=ABCMeta):
    ...
    def __init__(self, rotation: int = 0, image: Optional[Image] = None) -> None:
        ...
        self.__image: Image = (
            Image(pygame.Surface((10, 10))) if image is None else image
        )
    ...
    def act(self) -> None:
        ...
        raise NotImplementedError(
            "act method needs to be implemented in subclasses of pygreenfoot.Actor"
        )
    ...
    def repaint(self) -> None:
        ...
        if self.get_image() is not None:
            ...
            pos = [
                self.x * world.cell_size
                + max(0, (world.cell_size - self.get_image().width)) / 2
                + rel_pos[0] / 2,
                self.y * world.cell_size
                + max(0, (world.cell_size - self.get_image().height)) / 2
                + rel_pos[1] / 2,
            ]
    ...
    def get_intersecting_actors(
        self, type_: Optional[Type[_ActorType]]
    ) -> List[_ActorType]:
    ...
    def get_intersecting_actors_generator(
        self, type_: Optional[Type[_ActorType]]
    ...
    def get_actors_in_range(
        self, radius: int, type_: Optional[Type[_ActorType]] = None
    ) -> List[_ActorType]:
        """
    ...
    def get_actors_in_range_generator(
        self, radius: int, type_: Optional[Type[_ActorType]] = None
    ) -> Generator[_ActorType, None, None]:
    ...
    def get_actors_at_offset(
        self, dx: int, dy: int, type_: Optional[Type[_ActorType]]
    ) -> List[_ActorType]:
        """
    ...
    def get_actors_at_offset_generator(
        self, dx: int, dy: int, type_: Optional[Type[_ActorType]]
    ) -> Generator[_ActorType, None, None]:
        ...
        """
        yield from self.get_world().get_objects_at_generator(
            self.x + dx, self.y + dy, type_
        )
    ...
    def get_neighbours(
        ...
        cells: int,
        diagonal: bool = False,
        type_: Optional[Type[_ActorType]] = None,
    ) -> List[_ActorType]:
    ...
    def get_neighbours_generator(
        ...
        """
        if diagonal:
            yield from self.get_actors_in_range(
                cells * self.get_world().cell_size, type_
            )
        else:
    ...
    def is_at_edge(self) -> bool:
        ...
        world = self.get_world()
        return (
            not world._rect.contains(self._rect)
            or self.x == 0
            or self.y == 0
            or self.x == world.width - 1
            or self.y == world.height - 1
        )
    ...
    def remove_touching(
        self, type_: Type["Actor"], fail_silently: bool = False
    ) -> None:
